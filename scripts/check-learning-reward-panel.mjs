...
const bundle = await build({
  ...
  plugins: [{ name: "panel-boundaries", setup(builder) {
    ...
    builder.onLoad({ filter: /.*/, namespace: "fixture" }, args => ({ contents: args.path.endsWith("LocaleProvider")
      ? 'const tc = value => value; export const useLocale = () => ({tc});'
...
try {
  for (const failure of ["http", "network", "wrong-student"]) {
    ...
    await page.route("http://reward.test/**", async route => {
      ...
      if (path === "/api/teacher/classes") return route.fulfill({ json: { items: [] } });
