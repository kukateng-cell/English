-- Activation is the sole writer allowed to close the CURRENT source year as
-- it opens the PLANNED successor. Keep all other status transitions blocked.
...
BEGIN
  ...
  IF OLD.status <> NEW.status
     AND NOT (
       roster_activation_enabled()
       AND (
         (OLD.status = 'PLANNED'::"AcademicYearStatus" AND NEW.status = 'CURRENT'::"AcademicYearStatus")
         OR (OLD.status = 'CURRENT'::"AcademicYearStatus" AND NEW.status = 'CLOSED'::"AcademicYearStatus")
       )
     )
  THEN
