...
import { queryLearningRewardTimeline, readRewardRequest, recheckRewardAccess } from "@/lib/learning-reward-analytics";
...
function statusFor(code: string) {
  ...
  if (["QUERY_INVALID", "RANGE_OUTSIDE_CURRENT_YEAR"].includes(code)) return 422;
...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!id || Buffer.byteLength(id, "utf8") > 128) return NextResponse.json({ code: "STUDENT_NOT_FOUND" }, { status: 404, headers });
  try {
    const request = await readRewardRequest(req, { route: "TIMELINE" });
    const result = await queryLearningRewardTimeline({ userId: auth.userId, role: auth.role, studentId: id, request });
