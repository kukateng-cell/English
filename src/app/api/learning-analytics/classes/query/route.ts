...
import { readAnalyticsQuery, queryLearningAnalyticsClasses } from "@/lib/learning-analytics";
...
export async function POST(req: Request) {
  ...
  try { return NextResponse.json(await queryLearningAnalyticsClasses({ userId: auth.userId, role: auth.role, query: await readAnalyticsQuery(req, { route: "CLASSES" }) }), { headers }); }
  catch (error) { const code = error instanceof Error ? error.message : "INTERNAL_ERROR"; const status = ["PAYLOAD_TOO_LARGE", "ANALYTICS_SCOPE_TOO_LARGE"].includes(code) ? 413 : ["QUERY_INVALID", "RANGE_OUTSIDE_CURRENT_YEAR"].includes(code) ? 422 : ["CLASS_NOT_FOUND", "STUDENT_NOT_FOUND"].includes(code) ? 404 : code === "CURRENT_YEAR_UNAVAILABLE" ? 503 : code === "ANALYTICS_SCOPE_STALE" ? 409 : code === "ROLE_FORBIDDEN" ? 403 : code === "AUTH_REQUIRED" ? 401 : 500; return NextResponse.json({ code: status === 500 ? "INTERNAL_ERROR" : code }, { status, headers }); }
}
