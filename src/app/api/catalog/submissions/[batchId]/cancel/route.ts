...
import { cancelCatalogSubmissionBatch } from "@/lib/catalog/submission-server";
...
export async function POST(req: Request, { params }: { params: Promise<{ batchId: string }> }) {
  ...
  try {
    ...
    const batch = await cancelCatalogSubmissionBatch({ batchId, actorId: auth.actor.userId, expectedRevision: Number(body.expectedRevision) });
