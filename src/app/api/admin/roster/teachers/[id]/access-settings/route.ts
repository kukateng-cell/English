  ...
  try {
    const snapshot = await prisma.$transaction(async (tx) => {
      const [teacher, year] = await Promise.all([
        tx.user.findFirst({ where: { id, role: ROLES.TEACHER }, select: { id: true, accountName: true, status: true, teacherProfile: { select: { legalName: true, accessRevision: true, canResetStudentPassword: true, classAccess: { where: { schoolClass: { academicYearId } }, select: { classId: true, canViewProgress: true } } } } } }),
...
export async function PUT(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!Number.isInteger(expectedAccessRevision) || typeof canReset !== "boolean" || (acknowledge !== undefined && typeof acknowledge !== "boolean")) return rosterResponse("ACCESS_INPUT_INVALID", 422);
  ...
  try {
    const result = await prisma.$transaction(async (tx) => {
      ...
      const teacher = await tx.user.findFirst({ where: { id, role: ROLES.TEACHER }, select: { id: true, accountName: true, teacherProfile: { select: { accessRevision: true, canResetStudentPassword: true } } } });
      if (!teacher?.teacherProfile) throw new Error("TEACHER_NOT_FOUND");
      ...
      const changed = globalChanged || selectedChanged;
      if (globalChanged) await tx.teacherProfile.update({ where: { userId: id }, data: { canResetStudentPassword: canReset } });
      ...
      const audit = changed ? await tx.securityEvent.create({ data: securityEventData({ actorUserId: gate.auth.userId, subjectUserId: id, subjectAccount: teacher.accountName, eventType: "TEACHER_CLASS_ACCESS_CHANGED", ip: getClientIp(req.headers), metadata: { globalResetCapability: canReset, academicYearId: selectedYearId, selectedClassCount: selectedClassIds?.length ?? null } }) }) : null;
      const currentClasses = await tx.schoolClass.findMany({ where: { academicYear: { status: "CURRENT" }, active: true, teacherAccess: { some: { teacherId: id, canViewProgress: true } } }, select: { id: true } });
      const studentCount = await tx.studentEnrollment.count({ where: { status: "ACTIVE", academicYear: { status: "CURRENT" }, student: { user: { role: ROLES.STUDENT, status: "ACTIVE" } }, schoolClass: { is: { teacherAccess: { some: { teacherId: id, canViewProgress: true } } } } } });
      const updated = await tx.teacherProfile.findUniqueOrThrow({ where: { userId: id }, select: { accessRevision: true, canResetStudentPassword: true } });
      return { accessRevision: updated.accessRevision, canResetStudentPassword: updated.canResetStudentPassword, currentImpact: { classCount: currentClasses.length, studentCount }, selectedYear: selectedYearId ? { academicYearId: selectedYearId, classIds: selectedClassIds } : null, auditEventId: audit?.id ?? null };
