...
import { assertRosterSelectionCap, parseClassCode } from "@/lib/roster-domain";
...
export async function POST(req: Request) {
  ...
  try {
    const summary = await prisma.$transaction(async (tx) => {
      ...
      const enrollments = await tx.studentEnrollment.findMany({ where: { studentId: { in: ids }, academicYearId: year.id, status: "ACTIVE" }, select: { id: true, studentId: true, revision: true, classId: true } });
    ...
    if (error instanceof Error && ["BATCH_NOT_FOUND", "BATCH_INVALID", "CURRENT_YEAR_REQUIRED", "BATCH_EXPIRED", "STALE_PREVIEW", "IDEMPOTENCY_CONFLICT"].includes(error.message)) return response(error.message, error.message === "BATCH_NOT_FOUND" ? 404 : error.message === "BATCH_EXPIRED" ? 410 : 409);
...
export async function createBulkClassPreview(input: {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const enrollments = await tx.studentEnrollment.findMany({ where: { studentId: { in: input.studentIds.filter((id) => !excluded.has(id)) }, academicYearId: year.id, status: "ACTIVE", student: { user: { role: "STUDENT", status: "ACTIVE" } } }, select: { id: true, studentId: true, revision: true, grade: true, classId: true } });
