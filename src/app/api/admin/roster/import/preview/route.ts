...
function validateHeaders(rows: RosterCellRow[], entityType: "STUDENT" | "TEACHER") {
  ...
  const aliases = entityType === "STUDENT"
    ? [...ACCOUNT_HEADERS, ...STUDENT_NUMBER_HEADERS, ...LEGAL_NAME_HEADERS, ...NICKNAME_HEADERS, ...GRADE_HEADERS, ...CLASS_HEADERS, ...EMAIL_HEADERS]
...
function parseTeacherReset(value: string, templateVersion: "teacher-roster-v2" | "v1") {
  ...
  if (!/^(true|false)$/iu.test(trimmed)) return { value: undefined, error: "resetPasswordCapability 必須為 TRUE 或 FALSE" };
  ...
  try {
    ...
    const existing = await prisma.user.findMany({
      ...
      select: {
        ...
        studentProfile: { select: { legalName: true, nickname: true, nicknameNormalized: true, enrollments: { where: { academicYearId }, select: { id: true, grade: true, classId: true, studentNumber: true, status: true, schoolClass: { select: { classCode: true } } } } } },
    ...
    const existingStudentNumbers = entityType === "STUDENT" ? await prisma.studentEnrollment.findMany({
      // The database uniqueness indexes cover every enrollment lifecycle
      // state; preview must use the same scope so a CLOSED/ENDED row cannot
      // surprise the commit with a late P2002.
      where: { academicYearId, studentNumber: { not: null } },
      select: { studentNumber: true, grade: true, classId: true, schoolClass: { select: { classCode: true } }, student: { select: { user: { select: { id: true, accountName: true, accountNameCanonical: true } } } } },
    }) : [];
    ...
    const staged: StagedRosterRow[] = rows.map((row) => {
      ...
      if (accountSeen.has(accountName)) errors.push("檔案內帳號重複");
      ...
      if (mergeMode && existingUser && !legalName) errors.push("MERGE 必須提供真實姓名");
      ...
      if (contactEmail && emailSeen.has(contactEmail)) errors.push("檔案內聯絡電郵重複");
      if (contactEmail) emailSeen.add(contactEmail);
      const emailOwner = contactEmail ? existingByContact.get(contactEmail) : undefined;
      if (emailOwner && emailOwner.accountName !== accountName) errors.push("聯絡電郵已被其他帳號使用");
      if (existingUser && existingUser.role !== entityType) errors.push("帳號已屬於其他角色");
      if (existingUser && existingUser.role === entityType && !mergeMode) errors.push("帳號已經存在；僅新增模式不會覆蓋");
      ...
      if (entityType === "STUDENT") {
        ...
        if (mergeMode && existingUser && !rawNickname) errors.push("MERGE 必須提供暱稱");
        const grade = parseStudentGrade(field(row, GRADE_HEADERS));
        if (!grade) errors.push("年級必須為初一至高三");
        const rawClass = field(row, CLASS_HEADERS);
        const classCode = parseClassCode(rawClass);
        if (rawClass && !classCode) errors.push("班別必須為甲至辛");
        if (classCode && !classMap.has(`${grade}:${classCode}`)) errors.push("所選學年不存在該班級，請先建立班級");
        const rawStudentNumber = field(row, STUDENT_NUMBER_HEADERS);
        const clearStudentNumber = rawStudentNumber.trim() === "__CLEAR__";
        const parsedStudentNumber = clearStudentNumber ? null : parseStudentNumber(rawStudentNumber);
        if (rawStudentNumber.trim() && !clearStudentNumber && parsedStudentNumber === null) errors.push("學號必須是 1 至 999999 的正整數");
        if (existingUser && existingUser.role === entityType && !existingUser.studentProfile) errors.push("學生資料不完整");
        ...
        if (existingUser && mergeMode && currentEnrollment && currentEnrollment.grade !== grade && !classCode) errors.push("年級改變時必須同時提供新年級班別或明確未分班");
        const studentNumberAction = existingUser && mergeMode && !rawStudentNumber ? "PRESERVE" as const : clearStudentNumber ? "CLEAR" as const : "SET" as const;
        const effectiveStudentNumber = studentNumberAction === "PRESERVE" ? currentEnrollment?.studentNumber ?? null : parsedStudentNumber;
        const diff = existingUser?.studentProfile ? {
          ...
          studentNumber: { before: currentEnrollment?.studentNumber === null || currentEnrollment?.studentNumber === undefined ? null : String(currentEnrollment.studentNumber), after: effectiveStudentNumber === null ? null : String(effectiveStudentNumber) },
        } : undefined;
        const sameSelectedYear = Boolean(existingUser?.studentProfile && currentEnrollment && currentEnrollment.grade === grade && (currentEnrollment.schoolClass?.classCode ?? null) === (classCode ?? (currentEnrollment ? currentEnrollment.schoolClass?.classCode ?? null : null)) && currentEnrollment.studentNumber === effectiveStudentNumber);
        ...
        return { entityType, rowNumber, action, accountName, legalName, nickname: nickname.ok ? nickname.value : "", nicknameNormalized: nickname.ok ? nickname.normalized : "", contactEmail, contactEmailAction, grade, classCode, studentNumber: effectiveStudentNumber, studentNumberAction, errors, diff } satisfies StagedStudentRow;
      }
      ...
      if (Object.prototype.hasOwnProperty.call(row, "templateVersion") && templateVersion !== "teacher-roster-v2") errors.push("templateVersion 必須為 teacher-roster-v2");
      ...
      for (const access of parsedAccess.access) if (!classMap.has(`${access.grade}:${access.classCode}`)) errors.push("所選學年不存在教師權限班級");
    ...
    for (const row of staged) {
      if (row.entityType !== "STUDENT" || row.studentNumber === null || !row.grade) continue;
      const existingUser = existingByAccount.get(row.accountName);
      const currentEnrollment = existingUser?.studentProfile?.enrollments[0];
      const effectiveClassCode = row.classCode ?? currentEnrollment?.schoolClass?.classCode ?? null;
      const targetClass = effectiveClassCode ? classMap.get(`${row.grade}:${effectiveClassCode}`) : undefined;
      const key = studentNumberKey(targetClass?.id ?? null, targetClass ? null : effectiveClassCode, row.studentNumber);
      const previous = studentNumbers.get(key);
      if (previous !== undefined) {
        row.errors.push("STUDENT_NUMBER_CONFLICT");
        const prior = staged.find((candidate) => candidate.rowNumber === previous);
        if (prior) {
          prior.errors.push("STUDENT_NUMBER_CONFLICT");
          prior.action = "ERROR";
        }
        row.action = "ERROR";
      } else {
        const owner = existingStudentNumberOwners.get(key);
        const ownerAccount = owner?.accountNameCanonical ?? owner?.accountName;
        if (owner && ownerAccount !== row.accountName && owner.id !== existingUser?.id) {
          row.errors.push("STUDENT_NUMBER_CONFLICT");
          row.action = "ERROR";
        } else studentNumbers.set(key, row.rowNumber);
      }
    }
