...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const result = await prisma.$transaction(async (tx) => {
      ...
      const teachers = await tx.user.findMany({ where: { role: ROLES.TEACHER, teacherProfile: { isNot: null } }, orderBy: { id: "asc" }, select: { id: true, status: true, revision: true, teacherProfile: { select: { accessRevision: true, classAccess: { where: { schoolClass: { academicYearId: target.id } }, select: { classId: true, canViewProgress: true, canResetStudentPassword: true } } } } } });
      const coverage = classes.map((schoolClass) => {
        ...
        return { classId: schoolClass.id, classRevision: schoolClass.revision, grade: schoolClass.grade, classCode: schoolClass.classCode, viewTeacherIds: candidates.map((teacher) => teacher.id).sort(), resetTeacherIds: resetCandidates.map((teacher) => teacher.id).sort(), acknowledged: acknowledgedClassIds.has(schoolClass.id), teacherSnapshots: teachers.map((teacher) => ({ userId: teacher.id, status: teacher.status, revision: teacher.revision, accessRevision: teacher.teacherProfile?.accessRevision ?? null, access: (teacher.teacherProfile?.classAccess ?? []).filter((access) => access.classId === schoolClass.id).map((access) => ({ classId: access.classId, canViewProgress: access.canViewProgress, canResetStudentPassword: access.canResetStudentPassword })).sort((a, b) => a.classId.localeCompare(b.classId)) })) };
      });
