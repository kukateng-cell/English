...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (error instanceof Error && error.message === "PAYLOAD_TOO_LARGE") return json("PAYLOAD_TOO_LARGE", 413);
    return json("REQUEST_INVALID", 422);
  }
  if (!body || typeof body.resetPrecondition !== "string" || !body.resetPrecondition) return json("RESET_PRECONDITION_INVALID", 422);
