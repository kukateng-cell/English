...
function importErrorLabel(error: string) {
  const replacements: Array<[string, string]> = [
    ...
    ["templateVersion 必須為 teacher-roster-v2", "教師名單格式不正確，請下載最新模板"],
    ["LEGACY_RESET_SCOPE_UNSUPPORTED", "舊格式不支援重設密碼權限，請下載最新教師模板"],
...
export default function AdminRosterPage() {
  ...
  async function downloadTemplate() {
    ...
    if (!response.ok) throw new Error(await responseMessage(response));
    downloadBlob(await response.blob(), `${importType === "TEACHER" ? "教師" : "學生"}-名單模板.${importTemplateFormat}`);
  }
  ...
  return <div data-roster-page className="space-y-6">
    ...
    {tab === "students" && nextUserCursor ? <button className={secondaryButton} disabled={busy} onClick={() => void run(loadMoreUsers)}>{tc("載入更多學生／教師")}</button> : null}
