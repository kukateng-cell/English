...
type AccessState = { academicYear: Year; teacher: { teacherProfile: { accessRevision: number; classAccess: Array<{ classId: string; canViewProgress: boolean; canResetStudentPassword: boolean }> } }; classes: SchoolClass[] };

const STUDENT_FIELDS = ["accountName", "legalName", "nickname", "grade", "classCode", "contactEmail", "status", "mustChangePassword", "createdAt"];
const TEACHER_FIELDS = ["accountName", "legalName", "contactEmail", "status", "classAccess", "resetPasswordAccess", "createdAt"];
...
export default function AdminRosterPage() {
  ...
  const [accessSelection, setAccessSelection] = useState<Map<string, { view: boolean; reset: boolean }>>(new Map());
  ...
  async function loadTeacherAccess(id: string) {
    setTeacherAccessId(id); if (!id || !yearId) { setAccessState(null); return; }
    const response = await fetch(`/api/admin/roster/teachers/${id}/class-access?academicYearId=${encodeURIComponent(yearId)}`);
    if (!response.ok) throw new Error(await responseMessage(response));
    const payload = await response.json() as AccessState; setAccessState(payload); setAccessSelection(new Map(payload.teacher.teacherProfile.classAccess.map((item) => [item.classId, { view: item.canViewProgress, reset: item.canResetStudentPassword }])));
  }
  async function saveTeacherAccess() {
    if (!teacherAccessId || !accessState || !yearId) return;
    const access = [...accessSelection].filter(([, value]) => value.view).map(([classId, value]) => ({ classId, canViewProgress: value.view, canResetStudentPassword: value.reset }));
    const response = await rosterFetch(`/api/admin/roster/teachers/${teacherAccessId}/class-access`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ academicYearId: yearId, accessRevision: accessState.teacher.teacherProfile.accessRevision, access }) });
    if (!response.ok) throw new Error(await responseMessage(response));
    setMessage("教师班级权限已更新。 "); await loadTeacherAccess(teacherAccessId);
  }
  ...
  return <div data-roster-page className="space-y-6">
    ...
    {tab === "teachers" ? <section className="rounded-3xl border border-[var(--border)] bg-[var(--surface)] p-5 shadow-sm"><h2 className="text-[17px] font-bold text-[var(--text)]">{tc("教师名册与班级权限")}</h2><p className="mt-1 text-[13px] text-[var(--muted)]">{tc("教师只可查看获授权班级；重设密码是独立 capability。")}</p><select className={`${controlClass} mt-3`} value={teacherAccessId} onChange={(event) => void run(() => loadTeacherAccess(event.target.value))}><option value="">{tc("选择教师")}</option>{teachers.map((teacher) => <option key={teacher.id} value={teacher.id}>{teacher.legalName}（{teacher.accountName}）</option>)}</select>{accessState ? <div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{accessState.classes.map((schoolClass) => { const value = accessSelection.get(schoolClass.id) ?? { view: false, reset: false }; return <div key={schoolClass.id} className="rounded-2xl border border-[var(--border)] p-3 text-[12px]"><label className="flex items-center gap-2"><input type="checkbox" checked={value.view} onChange={(event) => setAccessSelection((current) => new Map(current).set(schoolClass.id, { view: event.target.checked, reset: event.target.checked ? value.reset : false }))} /><span>{tc(GRADE_LABELS[schoolClass.grade])}{tc(`${CLASS_LABELS[schoolClass.classCode]}班`)}{schoolClass.active ? "" : ` · ${tc("已停用")}`}</span></label><label className="mt-2 flex items-center gap-2 text-[var(--muted)]"><input type="checkbox" disabled={!value.view} checked={value.reset} onChange={(event) => setAccessSelection((current) => new Map(current).set(schoolClass.id, { view: value.view, reset: event.target.checked }))} />{tc("可重设密码")}</label></div>; })}<button className={`${primaryButton} sm:col-span-2 lg:col-span-3`} disabled={busy || currentYear?.status === "CLOSED"} onClick={() => void run(saveTeacherAccess)}>{tc("保存权限")}</button></div> : null}</section> : null}
