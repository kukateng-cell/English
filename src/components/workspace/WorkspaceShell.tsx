...
const ITEMS: Record<"teacher" | "admin", WorkspaceItem[]> = {
  ...
  admin: [
    ...
    { href: "/admin/words", label: "單詞庫", icon: "book" },
  ],
};
...
export default function WorkspaceShell({
  ...
  const roleTitle = role === "admin" ? "管理工作臺" : "教師工作臺";
  ...
  return (
    <div className="workspace-shell">
      <a className="skip-link" href="#workspace-main">{tc("跳到主要內容")}</a>
      <aside className="workspace-sidebar" aria-label={tc(role === "admin" ? "管理導航" : "教師導航") as string}>
        <BrandLockup />
        <div className="workspace-role-label">{tc(roleTitle)}</div>
        <nav className="workspace-nav" aria-label={tc("工作區導航") as string}>
          {items.map((item) => {
        ...
        <AccountControls user={user} compact homeHref={homeHref} homeLabel={role === "admin" ? "回到管理臺" : "回到教師臺"} />
      </aside>
      <div className="workspace-surface">
        <header className="workspace-mobile-header">
          <BrandLockup compact />
          <div className="workspace-mobile-title"><span>{tc(roleTitle)}</span><small>{tc("工作區")}</small></div>
          <AccountControls user={user} homeHref={homeHref} homeLabel={role === "admin" ? "回到管理臺" : "回到教師臺"} />
        </header>
        <nav className="workspace-mobile-nav" aria-label={tc("工作區導航") as string}>
          {items.map((item) => {
            const active = isActivePath(item.href);
