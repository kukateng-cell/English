  ...
  return (
    <div className="space-y-5">
      ...
      <div role="tablist" aria-label={tc("學生工作區") as string} className="flex w-fit max-w-full gap-2 rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-1.5">
        <button type="button" role="tab" aria-selected={view === "roster"} className={`ui-button ui-button-small ${view === "roster" ? "ui-button-primary" : "ui-button-secondary"}`} onClick={() => selectView("roster")}><Icon name="clipboard" size={16} />{tc("學生名冊")}</button>
      ...
      {view === "progress" ? <section className="ui-card ui-card-padding teacher-progress-controls">
        <div className="teacher-progress-controls-heading"><div><p className="text-sm font-semibold text-[var(--text)]">{tc("分析期間")}</p><p className="mt-1 text-xs text-[var(--muted)]">{tc("可選取最多八名學生比較；練習量與目前掌握分開顯示。")}</p></div><MetricDefinitionsHelp context="progress" /></div>
        <div className="teacher-progress-controls-fields">
        ...
        <div className="teacher-progress-controls-actions">
          <select aria-label={tc("統計分組")} value={exportGranularity} onChange={(event) => setExportGranularity(event.target.value as typeof exportGranularity)} className="h-10 rounded-xl border border-[var(--border)] bg-[var(--surface)] px-3 text-xs"><option value="DAY">{tc("每日")}</option><option value="WEEK">{tc("每週")}</option><option value="MONTH">{tc("每月")}</option></select>
          <select aria-label={tc("下載格式")} value={exportFormat} onChange={(event) => setExportFormat(event.target.value as typeof exportFormat)} className="h-10 rounded-xl border border-[var(--border)] bg-[var(--surface)] px-3 text-xs"><option value="XLSX">{tc("Excel（XLSX）")}</option><option value="CSV">{tc("CSV")}</option></select>
          <button type="button" className="ui-button ui-button-secondary ui-button-small" disabled={exporting} onClick={() => void exportReport("STUDENTS")}>{exporting ? tc("匯出中…") : tc("匯出學生報告")}</button>
          <button type="button" className="ui-button ui-button-secondary ui-button-small" disabled={exporting} onClick={() => void exportReport("CLASSES")}>{exporting ? tc("匯出中…") : tc("匯出班級報告")}</button>
      ...
      {view === "progress" && selectedIds.length ? <section className="ui-card ui-card-padding"><div className="flex flex-wrap items-center justify-between gap-2"><div><p className="text-sm font-bold text-[var(--primary)]">{tc("學生比較")}</p><h2 className="mt-1 text-xl font-black text-[var(--text)]">{fromDate} 至 {toDate}</h2></div><div className="flex items-center gap-2">{comparisonLoading ? <span className="text-xs text-[var(--muted)]">{tc("正在更新比較…")}</span> : null}<MetricDefinitionsHelp context="progress" /></div></div>{comparisonError ? <p className="mt-3 text-sm text-[var(--danger)]">{comparisonError}</p> : comparison.length ? <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{comparison.map((item) => <article key={item.id} className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-4"><h3 className="font-bold text-[var(--text)]">{item.nickname || item.legalName}</h3><p className="mt-1 text-xs text-[var(--muted)]">{item.accountName}</p><dl className="mt-3 grid grid-cols-2 gap-2 text-sm"><div><dt className="text-xs text-[var(--muted)]">{tc("練習")}</dt><dd className="font-semibold">{item.learningEncounterCount}</dd></div><div><dt className="text-xs text-[var(--muted)]">{tc("已計入測驗")}</dt><dd className="font-semibold">{item.effectiveReviewCount}</dd></div><div><dt className="text-xs text-[var(--muted)]">{tc("客觀測驗")}</dt><dd className="font-semibold">{item.objective.correctCount}/{item.objective.eligibleAttemptCount}</dd></div><div><dt className="text-xs text-[var(--muted)]">{tc("目前掌握")}</dt><dd className="font-semibold text-[var(--primary)]">{item.currentMastery.percent === null ? "—" : `${item.currentMastery.percent}%`}</dd></div></dl></article>)}</div> : <p className="mt-3 text-sm text-[var(--muted)]">{tc("未有符合目前篩選的比較資料")}</p>}<p className="mt-3 text-xs text-[var(--muted)]">{tc("比較只顯示獲授權學生；測驗數字保留答對數及作答分母。")}</p></section> : null}
