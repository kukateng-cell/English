...
function itemTitle(item: WorkItem, tc: (value: string) => string): string {
  if (item.type === "BATCH") return item.fileName || tc("批量提交");
...
function itemMeta(item: WorkItem, tc: (value: string) => string): string {
  if (item.type === "BATCH") return `${tc("批量提交")} · ${item.rowCount ?? 0} ${tc("行")} · ${item.status ?? ""}`;
...
export default function CatalogWorkItemsWorkspace({ bulkEnabled, onOpenCatalog, onOpenBatch, onRetryRequest }: {
  ...
  return <div className="space-y-5">
    <div className="flex flex-wrap items-start justify-between gap-3"><div><h1 className="text-[22px] font-bold tracking-[-0.03em] text-[var(--text)]">{tc("我的詞庫待辦")}</h1><p className="mt-1 text-sm text-[var(--muted)]">{tc("集中查看需要修正、需要審核、等待處理及最近結果；紅色數字只計算需要你處理的項目。")}</p></div><button type="button" className="ui-button ui-button-secondary ui-button-small" disabled={loading} onClick={() => void load()}>{tc("重新整理")}</button></div>
    {error ? <ErrorBanner message={error} onRetry={() => void load()} /> : null}
    {message ? <p role="status" className="rounded-xl bg-[var(--success-bg)] px-4 py-3 text-sm text-[var(--success)]">{message}</p> : null}
    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><div className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-4"><p className="text-xs text-[var(--muted)]">{tc("需要你處理")}</p><p className="mt-1 text-2xl font-bold text-[var(--danger)]">{data.counts.totalActionable}</p></div><div className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-4"><p className="text-xs text-[var(--muted)]">{tc("需要修改後重新提交")}</p><p className="mt-1 text-2xl font-bold text-[var(--text)]">{data.counts.requestsToRevise + data.counts.batchesToRevise}</p></div><div className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-4"><p className="text-xs text-[var(--muted)]">{tc("需要審核")}</p><p className="mt-1 text-2xl font-bold text-[var(--text)]">{data.counts.requestsToReview + data.counts.batchesToReview + data.counts.feedbackToReview}</p></div><div className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-4"><p className="text-xs text-[var(--muted)]">{tc("等待其他人處理")}</p><p className="mt-1 text-2xl font-bold text-[var(--text)]">{data.sectionTotals.waiting}</p></div></div>
    <section><h2 className="text-lg font-bold text-[var(--text)]">{tc("需要修改後重新提交")}</h2>{renderItems(data.needsRevision, "revision", data.sectionTotals.needsRevision)}</section>
