  ...
  return <div className="fixed inset-0 z-[60] flex items-end justify-center bg-black/35 sm:items-center sm:p-5" role="dialog" aria-modal="true" aria-labelledby="catalog-feedback-title">
    <section ref={dialogRef} tabIndex={-1} onKeyDown={handleKeyDown} className="max-h-[92vh] w-full max-w-xl overflow-y-auto rounded-t-3xl bg-[var(--surface)] p-5 shadow-2xl sm:rounded-3xl">
      <div className="flex items-start justify-between gap-3">
        <div><h2 id="catalog-feedback-title" className="text-xl font-bold text-[var(--text)]">{tc("提出詞庫意見")}</h2><p className="mt-1 text-sm text-[var(--muted)]">{tc("請簡要說明問題；意見不會直接改動學生詞庫，正式修改仍須審核。")}</p></div>
      ...
      <div className="mt-5 grid gap-4">
        ...
        <label className="grid gap-1 text-xs font-semibold text-[var(--muted)]">{tc("你發現了甚麼問題？")}<textarea className="min-h-28 rounded-xl border border-[var(--border)] bg-[var(--surface)] px-3 py-2 text-sm text-[var(--text)]" value={message} maxLength={2000} onChange={(event) => setMessage(event.target.value)} placeholder={tc("例如：這個中文解釋較深，A1 學生可能難以理解。") as string} /></label>
        <label className="grid gap-1 text-xs font-semibold text-[var(--muted)]">{tc("建議如何修改（可留空）")}<textarea className="min-h-20 rounded-xl border border-[var(--border)] bg-[var(--surface)] px-3 py-2 text-sm text-[var(--text)]" value={suggestedValue} maxLength={2000} onChange={(event) => setSuggestedValue(event.target.value)} /></label>
      </div>
