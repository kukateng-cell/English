...
function CatalogOverviewWorkspace({
  ...
  return <div className="space-y-5">
    <div className="flex flex-wrap items-start justify-between gap-3">
      ...
      <div className="flex flex-wrap gap-2"><button type="button" className="ui-button ui-button-primary" onClick={startCreate}>{tc("新增詞條")}</button>{bulkEnabled ? <><button type="button" className="ui-button ui-button-secondary" disabled={saving || exportSenseKeys.size === 0} onClick={() => void exportSelectedUpdates()}>{tc("匯出所選作 CSV 更新")} ({exportSenseKeys.size}/200)</button>{exportSenseKeys.size ? <button type="button" className="ui-button ui-button-quiet" disabled={saving} onClick={() => setExportSenseKeys(new Set())}>{tc("清除全部選取")}</button> : null}</> : null}</div>
...
export default function CatalogGovernanceWorkspace() {
  ...
  return <div className="mx-auto w-full max-w-[1500px] space-y-5 overflow-x-clip">
    ...
    {tab === "catalog" ? <CatalogOverviewWorkspace key={catalogResetVersion} bulkEnabled={bulkEnabled} historyEnabled={historyEnabled} onOpenHistory={(senseKey) => { setHistorySenseKey(senseKey); setTab("history"); }} /> : tab === "bulk" ? <CatalogBulkSubmissionWorkspace canReview={canReview} actorUserId={actorUserId} initialBatchId={bulkBatchId} /> : <CatalogHistoryWorkspace canReview={canReview} bulkEnabled={bulkEnabled} initialSenseKey={historySenseKey} onBackToCatalog={() => { setHistorySenseKey(null); setTab("catalog"); }} onOpenCorrectiveBatch={(batchId) => { setBulkBatchId(batchId); setTab("bulk"); }} />}
  </div>;
}
