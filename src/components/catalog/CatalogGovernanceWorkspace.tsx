...
function CatalogOverviewWorkspace({
  ...
  return <div className="space-y-5">
    ...
    <div aria-busy={loading || loadingMore}>{loading ? <p role="status" className="rounded-2xl border border-dashed border-[var(--border)] bg-[var(--surface)] px-4 py-10 text-center text-sm text-[var(--muted)]">{tc("正在更新詞庫結果…")}</p> : rows.length ? <div className="space-y-2">{rows.map((row) => <article key={row.id} className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-4 shadow-sm"><div className="grid gap-3 lg:grid-cols-[minmax(180px,1.1fr)_minmax(220px,1.6fr)_100px_150px_auto] lg:items-center"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><strong className="break-words text-[17px] text-[var(--text)]">{row.term || tc("未完成詞條")}</strong><span className={`rounded-full px-2 py-0.5 text-[10px] font-semibold ${statusClass(row.status)}`}>{statusLabel(row.status, tc)}</span></div><p className="mt-1 break-all text-[11px] text-[var(--muted)]">{row.senseKey ?? tc("尚未建立 sense key")}</p></div><div className="min-w-0"><p className="line-clamp-2 text-sm text-[var(--text)]">{row.definitionZh || tc("尚未填寫中文釋義")}</p><p className="mt-1 text-xs text-[var(--muted)]">{row.partOfSpeech || "—"} · {row.level || "—"} · {tc(row.category || "other")}</p></div><div className="text-xs text-[var(--muted)]"><p>{row.enableEnToZh ? tc("英譯中") : tc("英譯中停用")}</p><p>{row.enableZhToEn ? tc("中譯英") : tc("中譯英停用")}</p></div><div className="text-xs text-[var(--muted)]"><p>{row.primaryDisposition === "VALIDATION_FAILED" ? tc("需修訂 validator 問題") : row.eligibilityResult === "DRAFT_BLOCKED" ? tc("出題方向未就緒") : row.approvedRevisionId ? `${tc("已批准 revision")} ${row.revision ?? "—"}` : tc("未批准")}</p><p>{row.pendingRequest ? tc("已有待審核修改") : row.sourceFile ? `${row.sourceFile}:${row.sourceRow}` : tc("治理草稿")}</p></div><div className="flex flex-wrap gap-2 lg:justify-end">{hasCatalogExportTarget(row) ? <label className="flex items-center gap-1.5 text-xs text-[var(--muted)]"><input type="checkbox" checked={exportSenseKeys.has(row.senseKey!)} disabled={Boolean(row.pendingRequest) || (!exportSenseKeys.has(row.senseKey!) && exportSenseKeys.size >= 200)} onChange={(event) => toggleExportSelection(row.senseKey!, event.target.checked)} />{tc("選取匯出")}</label> : catalogExportAvailability(row) === "REQUIRES_GOVERNED_REVISION" ? <span role="note" className="max-w-44 text-xs leading-5 text-[var(--muted)]">{tc("請先查看／修改並建立詞義版本，之後才可批量匯出。")}</span> : catalogExportAvailability(row) === "REVISION_UNAVAILABLE" ? <span role="note" className="max-w-44 text-xs leading-5 text-[var(--danger)]">{tc("詞義版本未就緒；請重新載入，如持續出現請通知管理員。")}</span> : <span role="note" className="max-w-44 text-xs leading-5 text-[var(--muted)]">{tc("未有 sense key；請用 CSV 批量提交修正來源資料。")}</span>}{row.senseKey ? <><button type="button" className="ui-button ui-button-quiet ui-button-small" onClick={() => onOpenHistory(row.senseKey!)}>{tc("查看歷史")}</button><button type="button" className="ui-button ui-button-secondary ui-button-small" onClick={() => void openDetail(row)}>{tc("查看／修改")}</button></> : null}{row.validationErrors.length ? <span className="rounded-full bg-[var(--danger-bg)] px-2 py-1 text-[10px] text-[var(--danger)]">{row.validationErrors.length} {tc("個問題")}</span> : null}</div></div></article>)}</div> : <p className="rounded-2xl border border-dashed border-[var(--border)] bg-[var(--surface)] px-4 py-10 text-center text-sm text-[var(--muted)]">{tc("目前篩選沒有詞條。")}</p>}</div>
