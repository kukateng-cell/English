  ...
  async function reviewRequest(request: PendingRequest, decision: "APPROVE" | "REJECT") {
    ...
    try {
      ...
      if (isCurrentDialogIntent(reviewIntent)) {
        setReviewNote("");
        if (selectedRequestIdAtStart === request.id && selectedPendingRequestIdRef.current === request.id) {
