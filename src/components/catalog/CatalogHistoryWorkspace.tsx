...
export default function CatalogHistoryWorkspace({ canReview, bulkEnabled, onOpenCorrectiveBatch, onBackToCatalog, initialSenseKey }: { canReview: boolean; bulkEnabled: boolean; onOpenCorrectiveBatch: (batchId: string) => void; onBackToCatalog: () => void; initialSenseKey?: string | null }) {
  const { tc } = useLocale();
  ...
  return <div className="space-y-4">
    ...
    <div className="space-y-3" aria-live="polite">{items.map((entry) => {
      ...
      return <article key={entry.feedEntryId} className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-4">
        ...
        {expanded === entry.feedEntryId ? <div className="mt-4 space-y-3 border-t border-[var(--border)] pt-4">{entry.baseline ? <pre className="max-h-64 overflow-auto whitespace-pre-wrap rounded-xl bg-[var(--border-soft)] p-3 text-xs text-[var(--text)]">{JSON.stringify(entry.baseline.report, null, 2)}</pre> : <>{entry.batch ? <BatchTimeline batch={entry.batch} tc={tc} /> : null}{entry.batch?.retryOfBatchId ? <p className="rounded-xl bg-[var(--warning-bg)] px-3 py-2 text-xs text-[var(--warning)]">{tc("此批次是被拒絕／要求修正批次的修正版；原批次紀錄仍然保留。")} <span className="font-mono">{entry.batch.retryOfBatchId}</span></p> : null}{entry.batch?.correctiveOfBatchId ? <p className="rounded-xl bg-[var(--border-soft)] px-3 py-2 text-xs text-[var(--muted)]">{tc("此批次是已套用批次的反向修正預覽。")} <span className="font-mono">{entry.batch.correctiveOfBatchId}</span></p> : null}{changes.map((change) => <ChangeDiff key={change.id} change={change} tc={tc} />)}</>}{entry.batch && batchChildCursor[entry.batch.id] ? <button type="button" className="ui-button ui-button-secondary ui-button-small" disabled={loading} onClick={() => void loadBatchChildren(entry.batch!.id, true)}>{tc("載入更多批次子項")}</button> : null}{canReview && bulkEnabled && entry.batch?.status === "COMMITTED" ? <button type="button" className="ui-button ui-button-secondary" disabled={correcting === entry.batch.id} onClick={() => void createCorrectivePreview(entry.batch!.id)}>{correcting === entry.batch.id ? tc("建立中…") : tc("建立反向修正預覽")}</button> : null}</div> : null}
      </article>;
    })}</div>
