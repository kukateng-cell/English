...
export default function StudentDashboard({ userId }: { userId: string }) {
  ...
  if (error && !data) {
    return <div className="student-content-narrow"><StatusBanner variant="error" message={tc(error)} action={<Button variant="quiet" size="small" onClick={() => setReloadKey((key) => key + 1)}>{tc("重试")}</Button>} /></div>;
  }
  if (!data) {
    return <div className="student-content-narrow"><PageHeader title={tc("今天继续学习")} description={tc("正在读取你的学习概览")} /><div className="dashboard-skeleton-grid"><Skeleton className="h-32" /><Skeleton className="h-32" /><Skeleton className="h-32" /></div><Skeleton className="mt-5 h-56" label={tc("正在加载学习概览")} /></div>;
  }
  ...
  return (
    <div className="student-content-narrow">
      ...
      <Card className="dashboard-library-card" padded>
        ...
        <p className="ui-field-helper">{tc("已学使用认字口径；长期掌握使用 SM-2 间隔口径。")} </p>
      </Card>
