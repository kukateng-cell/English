...
export default function AnalyticsDashboard({ role }: { role: Role }) {
  ...
  return <div className="space-y-5">
    <header className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-sm font-bold text-[var(--primary)]">{tc(role === "ADMIN" ? "管理工作台" : "教師工作台")}</p><h1 className="mt-1 text-3xl font-black tracking-tight text-[var(--text)]">{tc("學習分析")}</h1><p className="mt-1 text-sm text-[var(--muted)]">{tc("按目前班級成員計算；活動期間與目前掌握狀態分開顯示。")}</p></div><div className="flex flex-wrap items-center gap-2"><select aria-label={tc("報告格式")} value={exportFormat} onChange={(event) => setExportFormat(event.target.value as typeof exportFormat)} className="h-9 rounded-xl border border-[var(--border)] bg-[var(--surface)] px-2 text-xs"><option value="XLSX">XLSX</option><option value="CSV">CSV</option></select><button type="button" className="ui-button ui-button-secondary ui-button-small" disabled={Boolean(exporting)} onClick={() => void exportReport("STUDENTS")}>{exporting === "STUDENTS" ? tc("匯出中…") : tc("匯出學生報告")}</button><button type="button" className="ui-button ui-button-secondary ui-button-small" disabled={Boolean(exporting)} onClick={() => void exportReport("CLASSES")}>{exporting === "CLASSES" ? tc("匯出中…") : tc("匯出班級報告")}</button><Link href={role === "ADMIN" ? "/admin/users" : "/teacher/roster"} className="ui-button ui-button-secondary ui-button-small"><Icon name="users" size={16} />{tc("查看名單")}</Link></div></header>
