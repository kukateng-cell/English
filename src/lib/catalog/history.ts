...
import { readCatalogWorkspaceVersion } from "./workspace-version";
...
export async function ensureCatalogHistoryFeed(): Promise<void> {
  await prisma.$transaction(async (tx) => {
    const baseline = await tx.catalogImportBatch.findFirst({
      where: { status: "READY" },
    ...
    if (baseline) {
      await tx.catalogHistoryFeedEntry.upsert({
        ...
        create: {
          ...
          initialImportBatchId: baseline.id,
        },
        update: {},
      });
    }
    ...
    for (const request of requests) {
      await tx.catalogHistoryFeedEntry.create({
        data: {
          occurredAt: request.createdAt,
          sourceKind: "STANDALONE_REQUEST",
...
function payloadForVisibility(
  value: Prisma.JsonValue | null,
  visibility: CatalogHistoryVisibility,
) {
...
function requestVisibility(
  request: { proposerId: string; status: string },
  actorId: string,
  canReview: boolean,
): CatalogHistoryVisibility | null {
...
function requestDto(
  request: Prisma.CatalogChangeRequestGetPayload<{
...
const feedInclude = {
  ...
  initialImportBatch: {
    select: { id: true, sourceDigest: true, report: true, createdAt: true },
  },
} as const;

type FeedEntry = Prisma.CatalogHistoryFeedEntryGetPayload<{
...
function feedDto(entry: FeedEntry, actorId: string, canReview: boolean) {
  if (entry.sourceKind === "STANDALONE_REQUEST" && entry.request) {
    ...
    return {
      ...
      request: requestDto(entry.request, visibility),
    };
  }
  if (entry.sourceKind === "BATCH" && entry.submissionBatch) {
    ...
    const visibility: CatalogHistoryVisibility = canReview
      ? "REVIEWER"
      : owner
        ? "OWNER"
        : "PUBLIC_APPROVED";
  ...
  if (entry.sourceKind === "INITIAL_BASELINE" && entry.initialImportBatch) {
    return {
      ...
      baseline: {
        ...
        report: canReview
          ? entry.initialImportBatch.report
          : {
              rows:
                typeof (
                  entry.initialImportBatch.report as Record<
                    string,
                    unknown
                  > | null
                )?.rows === "number"
                  ? (entry.initialImportBatch.report as Record<string, unknown>)
                      .rows
                  : undefined,
              status: "READY",
            },
        ...
        ...(canReview
          ? { sourceDigest: entry.initialImportBatch.sourceDigest }
          : {}),
      },
    };
  }
...
export async function listCatalogHistory(input: {
  ...
  if (input.cursor && !cursor)
    throw new Error("CATALOG_HISTORY_CURSOR_INVALID");
  const scope = input.canReview ? "REVIEWER" : `TEACHER:${input.actorId}`;
  const fingerprint = catalogHistoryFilterFingerprint(filters);
  if (cursor && (cursor.scope !== scope || cursor.fingerprint !== fingerprint))
    throw new Error("CATALOG_HISTORY_CURSOR_INVALID");
  if (filters.actor && !input.canReview)
    throw new Error("CATALOG_HISTORY_FILTER_FORBIDDEN");
  ...
  const requestFilter: Prisma.CatalogChangeRequestWhereInput = {
    ...
    ["PENDING", "APPROVED", "REJECTED", "CANCELLED"].includes(status)
      ...
      : {}),
    ...(kind && ["CREATE", "UPDATE", "RETIRE", "REACTIVATE"].includes(kind)
      ? { kind: kind as "CREATE" | "UPDATE" | "RETIRE" | "REACTIVATE" }
      : {}),
    ...(level && ["A1", "A2", "B1", "B2"].includes(level)
      ? {
          OR: [
            { beforeLevelSnapshot: level as "A1" | "A2" | "B1" | "B2" },
            { afterLevelSnapshot: level as "A1" | "A2" | "B1" | "B2" },
          ],
        }
      : {}),
    ...(category
      ? {
          OR: [
            { beforeCategorySnapshot: category },
            { afterCategorySnapshot: category },
          ],
        }
      : {}),
    ...(catalogKey ? { catalogKey } : {}),
    ...(senseKey ? { senseKey } : {}),
    ...(actorSearch && input.canReview
      ? {
          OR: [
            {
              proposer: {
    ...
    ...(search
      ? {
          OR: [
            { beforeNormalizedTermSnapshot: { contains: search } },
            { afterNormalizedTermSnapshot: { contains: search } },
            {
  ...
  const lowerSourceKinds = cursor
    ? cursor.sourceKind === "INITIAL_BASELINE"
      ...
      : cursor.sourceKind === "BATCH"
        ...
        : []
    : [];
  const cursorCondition: Prisma.CatalogHistoryFeedEntryWhereInput | null =
    cursor
      ? {
          OR: [
            { occurredAt: { lt: new Date(cursor.occurredAt) } },
            ...(lowerSourceKinds.length
              ? [
                  {
                    occurredAt: new Date(cursor.occurredAt),
                    sourceKind: { in: [...lowerSourceKinds] },
                  },
                ]
              : []),
            {
              occurredAt: new Date(cursor.occurredAt),
              sourceKind: cursor.sourceKind as
                | "STANDALONE_REQUEST"
                | "BATCH"
                | "INITIAL_BASELINE",
              id: { lt: cursor.id },
    ...
    request: {
      is: input.canReview
        ...
        : {
            AND: [
              requestFilter,
              { OR: [{ proposerId: input.actorId }, { status: "APPROVED" }] },
            ],
          },
    },
  };
  const batchMatch: Prisma.CatalogHistoryFeedEntryWhereInput = {
    submissionBatch: {
      is: input.canReview
        ? {
            ...(batchId ? { id: batchId } : {}),
            proposalGroups: { some: { changeRequest: { is: requestFilter } } },
          }
        : {
            OR: [
              {
                ...
                ...(batchId ? { id: batchId } : {}),
                proposalGroups: {
                  some: { changeRequest: { is: requestFilter } },
                },
              },
              {
                proposerId: { not: input.actorId },
                status: "COMMITTED",
                ...(batchId ? { id: batchId } : {}),
                proposalGroups: {
                  some: {
                    changeRequest: {
                      is: { AND: [requestFilter, { status: "APPROVED" }] },
                    },
                  },
                },
              },
  ...
  const filterCondition: Prisma.CatalogHistoryFeedEntryWhereInput | null =
    filterActive ? { OR: [standaloneMatch, batchMatch] } : null;
  const where: Prisma.CatalogHistoryFeedEntryWhereInput = {
    occurredAt: {
      lte: dateTo && dateTo < new Date(cutoff) ? dateTo : new Date(cutoff),
      ...(dateFrom ? { gte: dateFrom } : {}),
    },
    ...(sourceKind &&
    ["STANDALONE_REQUEST", "BATCH", "INITIAL_BASELINE"].includes(sourceKind)
      ? {
    ...
    AND: [
      ...
      (item): item is Prisma.CatalogHistoryFeedEntryWhereInput => item !== null,
    ),
  };
...
export async function getCatalogHistoryEntry(input: {
  feedEntryIdOrRequestId: string;
  actorId: string;
  canReview: boolean;
}) {
  const entry = await prisma.catalogHistoryFeedEntry.findFirst({
    where: {
      OR: [
        { id: input.feedEntryIdOrRequestId },
        { requestId: input.feedEntryIdOrRequestId },
      ],
    },
    include: feedInclude,
  });
  ...
  const request = await prisma.catalogChangeRequest.findUnique({
    where: { id: input.feedEntryIdOrRequestId },
  ...
  return {
    ...
    sourceKind: request.submissionProposalGroupId
      ? "BATCH"
      : "STANDALONE_REQUEST",
...
export async function getCatalogHistoryBatchChildren(input: {
  ...
  const batch = await prisma.catalogSubmissionBatch.findUnique({
    ...
    select: { proposerId: true, status: true },
  });
  if (!batch) throw new Error("CATALOG_HISTORY_NOT_FOUND");
  const visibility: CatalogHistoryVisibility | null = input.canReview
    ? "REVIEWER"
    : batch.proposerId === input.actorId
      ? "OWNER"
  ...
  if (input.cursor && !cursorId)
    throw new Error("CATALOG_HISTORY_CURSOR_INVALID");
  const rows = await prisma.catalogSubmissionProposalGroup.findMany({
    where: {
      batchId: input.batchId,
      changeRequest: {
        is: visibility === "PUBLIC_APPROVED" ? { status: "APPROVED" } : {},
      },
    },
  ...
  return {
    items: rows.slice(0, limit).flatMap((group) =>
      group.changeRequest
        ? [
            {
    ...
    nextCursor: hasMore
      ? encodeCatalogBatchChildCursor(input.batchId, rows[limit - 1]!.id)
      : null,
  };
}

export async function getCatalogSenseHistory(input: {
  senseKey: string;
  actorId: string;
  canReview: boolean;
  cursor?: string | null;
  ...
  if (!input.senseKey || input.senseKey.length > 191)
    throw new Error("CATALOG_HISTORY_FILTER_INVALID");
  if (
    input.limit !== undefined &&
    (!Number.isInteger(input.limit) || input.limit < 1)
    ...
    throw new Error("CATALOG_HISTORY_FILTER_INVALID");
  const limit = Math.min(input.limit ?? 25, 50);
  const cursor = decodeCatalogSenseHistoryCursor(input.cursor);
  if (input.cursor && !cursor)
    throw new Error("CATALOG_HISTORY_CURSOR_INVALID");
  const initialVersion = await readCatalogWorkspaceVersion();
  const scope = catalogSenseHistoryScope(
    input.canReview,
    input.actorId,
    initialVersion.signature,
  );
  if (
    cursor &&
    (cursor.senseKey !== input.senseKey || cursor.scope !== scope)
  ) {
    throw new Error("CATALOG_HISTORY_CURSOR_CONTEXT_MISMATCH");
  }
  const cutoff = cursor?.cutoff ?? new Date().toISOString();
  const sense = await prisma.wordSense.findUnique({
  ...
  const visible = requests.flatMap((request) => {
    const visibility = requestVisibility(
      request,
      input.actorId,
      input.canReview,
    );
    return visibility
      ? [{ request, dto: requestDto(request, visibility) }]
      : [];
  });
  const hasMore = visible.length > limit;
  const page = visible.slice(0, limit);
  const last = page.at(-1)?.request;
  const finalVersion = await readCatalogWorkspaceVersion();
  if (finalVersion.signature !== initialVersion.signature) {
    throw new Error("CATALOG_READ_STALE");
  }
  return {
    items: page.map((item) => item.dto),
    snapshotCutoff: cutoff,
