...
test("admin roster completes the year rollover workflow on a disposable fixture", async ({ page, browser }) => {
  ...
  try {
    ...
    const changeStudentPasswordResponse = await studentPage.request.post("/api/reset-password", { headers: studentHeaders, data: { currentPassword: rotatedCredential!.temporaryPassword, newPassword: studentNewPassword } });
    expect(changeStudentPasswordResponse.ok(), await changeStudentPasswordResponse.text()).toBeTruthy();
