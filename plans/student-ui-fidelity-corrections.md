...
| 2026-08-12 | Phase 0 | production `/study` baseline 已完成 | `npm run build` 成功（38 routes）；production server、登入 student、`zh-Hant`、390×844 顯示真實 `green` 字卡，`navs=[]`、shell=`student-shell is-immersive`、`scrollWidth=390`。現行卡面 `x=16,y=229.39,w=358,h=480`；卡內 actions 48px 高，位置約 `y=640.39`；標題仍是「今日學習 · 認識這個單詞嗎？」。截圖：`plans/archive/evidence/playwright/phase0/learn-prod-390x844.png` |
| 2026-08-12 | Phase 0 | production 首頁 spacing baseline 已完成 | 390×844 loaded real-data state：header→banner 32px、banner→next-session 0px、next-session→stats 24px、stats→library 24px、library→links 24px；direct child margin 由多個 child 自行持有。bottom nav `x=0,y=779,w=390,h=65,z=25,padding-bottom=6px`，`scrollWidth=390`。截圖：`plans/archive/evidence/playwright/phase0/home-prod-390x844.png` |
...
| 2026-08-12 | Phase 2 | 視覺 evidence | `plans/archive/evidence/playwright/phase2/study-nav-mobile-390x844.png`、`study-nav-desktop-1440x900.png` 已以 production build、real authenticated student、real queue 目視核對；mobile 四項 nav、active 學習、card/actions、safe-area 與語速控件沒有互相遮擋；desktop rail 與 study surface 同時可見 |
...
| 2026-08-12 | Phase 3 | Visual／accessibility evidence | `plans/archive/evidence/playwright/phase3/home-spacing-mobile-390x844.png`、`home-spacing-tablet-820x1180.png`、`home-spacing-desktop-1440x900.png` 已以 real authenticated data 產生並目視核對；mobile、tablet、desktop 的 section rhythm、固定 nav 及 content separation 可解釋。axe WCAG 2A/2AA `/`、`/words`、`/stats`：兩個 project 均 0 violations |
...
| 2026-08-12 | Phase 4 | Visual／accessibility evidence | `plans/archive/evidence/playwright/phase4/learn-card-mobile-390x844.png`、`learn-card-tablet-820x1180.png`、`learn-card-desktop-1440x900.png` 以 real authenticated student、real queue 產生並目視核對；study-card fidelity：8 passed、1 desktop-only capture skipped；desktop/mobile real-data structure、dark/reduced-motion/Forced Colors、geometry、axe WCAG 2A/2AA 均通過 |
...
| 2026-08-12 | Phase 5 | Action visual／interaction evidence | `plans/archive/evidence/playwright/phase5/learn-card-mobile-390x844.png`、`learn-card-tablet-820x1180.png`、`learn-card-desktop-1440x900.png` 已以 real authenticated queue 產生並目視核對；`study-action-fidelity` desktop/mobile：7 passed，覆蓋卡外 DOM 關係、同寬／60px／18px geometry、mobile nav 不遮擋、focus、ArrowLeft/Right、left help flow、right quiz flow；study-card fidelity 全頁 axe 及 dark/reduced-motion/Forced Colors：8 passed、1 capture skipped |
...
| 2026-08-12 | Phase 6 | 最終 responsive／visual evidence | 新增 `tests/e2e/student-ui-final-qa.spec.ts` 及 `student-final-qa` project；final QA 覆蓋 `/`、`/study`、`/words`、`/stats` 的 320×568、360×800、390×844、430×932、844×390、600×960、820×1180、1024×768、1366×768、1920×1080，檢查 nav visibility／44px targets、safe-area、horizontal overflow、card actions／fixed nav／speech-rate intersection；matrix 通過。核心 screenshots：`plans/archive/evidence/playwright/phase6/home-mobile-390x844.png`、`home-tablet-820x1180.png`、`home-desktop-1440x900.png`、`learn-mobile-390x844.png`、`learn-tablet-820x1180.png`、`learn-desktop-1440x900.png`；補充 `home-mobile-320x568.png`、`learn-mobile-320x568.png` |
