...
搜尋採canonical／case-insensitive比對，request body及response只用allowlist；application／proxy logs不得記錄raw search value。
...
Raw out-of-band profile SQL不屬支援writer且不能擴大server authorization；conformance inventory阻止application route繞過service。
...
| 有效客觀評測 | `ReviewEvent.eventKind=REVIEW`、`isHistorical=false`；V2另要求既有objective provenance完整，V1沿用已接受的REVIEW event |
...
- reset transaction 內重驗教師 global capability、class access、學生 CURRENT enrollment、status及 revisions。
...
- UI先顯示帶真名、學生證及「舊密碼／session立即失效」文案的確認dialog；pending期間disable同一target action。
  ...
  `RECENT_AUTH_REQUIRED`開啟既有reauth modal，target只留在component memory；成功後最多自動送出一次原本尚未執行的request，
  不把target／password放URL、storage或log。404／409／429／503各有可理解錯誤，唔會重試bcrypt storm。
...
- legalName、accountName只可出現在授權 teacher roster／detail；排行榜等公開 surface仍只用 nickname。
...
`ACTIVATION_PREVIEW_STALE`／`RESET_CREDENTIAL_STALE`、422 `QUERY_INVALID`／`CURSOR_INVALID`、429 `TEACHER_RESET_RATE_LIMITED`、503
`CURRENT_YEAR_UNAVAILABLE`／`RATE_LIMIT_BACKEND_UNAVAILABLE`／`AUDIT_BACKEND_UNAVAILABLE`。Response不回傳raw Prisma／SQL。
...
{ viewMode, scope:{academicYearId,grade,classId,accessRevision,rosterRevision},
  items:[{id,accountName,legalName,nickname,grade,classId:null|string,classCode:null|ClassCode,lastActivityAt,canResetStudentPassword}],
...
{ viewMode, scope, items:[{id,accountName,legalName,nickname,grade,classId,classCode,
  masteredWords,totalWords,masteryPercent,effectiveObjectiveEventCount,lastActivityAt,dueReviewCount,byLevel}],
  nextCursor, generatedAt }
```
...
- `canResetStudentPassword`（server effective result）；
...
- [x] 兩個獨立Subagents各自完整審查相同全份計劃；兩份P0／P1 findings已全部納入Revision 2，並反覆重讀至同一最新contract均PASS。
...
驗收：產品決定、route table、DTO及授權 predicate沒有歧義，未開始 schema／UI 實作前已有失敗基線測試。
...
- [ ] Reset確認dialog列明失效影響；新增shared limiter、client pending guard、server credential CAS、recent-auth modal及一次重送。
...
- [ ] 執行unit、lint、typecheck、migration、focused API及browser tests。
...
| Roster search | accountName／legalName／nickname、繁簡文字、空白、無結果、跨頁、重複姓名、前置零帳號 |
...
| Metrics | V1/V2、StudyDay、self-rating only、objective event、historical/bridge排除、0 words／students、Asia/Shanghai日界、mastery分母／due |
| Class summary | 0學生、0活動、多班、同一學生唯一CURRENT enrollment、逐班合計與detail一致、unassigned admin count |
| Password lifecycle | confirm、recent-auth expiry／一次續接、四維limiter、double-click、撤權race、one-time response、must-change、session revoke |
...
建議驗證指令按風險分層，不需要每個UI小改都重跑scale suite：
...
另加focused Playwright teacher/admin workspace spec；只有schema／permission workflow整合完成後跑一次相應browser flow，
不重跑與本改動無關的500-row import、5,000-row export／activation或card-motion suite。
...
- 獲實作批准後才把狀態改為「進行中」及修改程式；目前只保存已review定稿。
...
- [ ] 所有global capability初始false，沒有舊per-class→global自動擴權。
...
- [ ] reset recent-auth、rate limit、credential revision、session revoke、one-time secret及audit完整。
...
各自回覆`PASS`，P0／P1／mandatory P2均為0。其後只加入本段review evidence及狀態／索引紀錄，沒有改動§1–14 contract。
