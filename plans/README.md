...
| 文件 | 類型 | 狀態 | 說明 |
|---|---|---|---|
| [study-stream-retention-and-recovery-audit.md](./study-stream-retention-and-recovery-audit.md) | V2 審核修正 | 已完成（第七輪本地驗證；hosted CI 待推送後） | 追加 Objective Probe 已答未確認 feedback 跨 session 到期續接；雙 reviewer 及本地 DB／browser 回歸完成 |
