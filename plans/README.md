...
| [word-catalog-governance-and-lifecycle.md](./word-catalog-governance-and-lifecycle.md) | 實施計劃 | 進行中（Phase 0；no-prompt 規則已確認） | 建立 sense-level 詞庫、39 欄 `word-catalog-v1`（兩個 prompt 欄保留但必須留空）、每方向 5–6 個人工干擾項及同詞跨 sense 正解排除、immutable approved revision、多人 conflict 檢查、老師 capability 四眼審核、一般停用／緊急撤回及可追溯 catalog revision；schema、功能、轉換及 production 尚未開始 |
