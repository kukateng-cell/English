...
| [word-catalog-governance-and-lifecycle.md](./word-catalog-governance-and-lifecycle.md) | 實施計劃 | 進行中（治理工作區及正式 baseline 已完成 local verification） | 建立 sense-level 詞庫、39 欄 `word-catalog-v1`、分方向人工干擾池、immutable approved revision及老師 capability 四眼審核；現已完成治理 API／UI、學生及統計 current reader、V1 projection，以及所有環境一致的 digest-bound 5,469 ACTIVE／107 DRAFT baseline；CSV preview／commit、audit history、rate limit及 production仍待後續 |
| [word-catalog-bulk-submission-and-history.md](./word-catalog-bulk-submission-and-history.md) | 實施計劃 | 待審批（兩個獨立 reviewer PASS） | 延續詞庫治理工作區，規劃 `word-catalog-v1` CSV preview／many-to-one conflict resolution／原子草稿提交／批次審核，以及按權限可搜尋 before／after、revision及audit timeline的詞條修改歷史界面 |
