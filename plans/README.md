...
| [class-roster-import-and-access-control.md](./class-roster-import-and-access-control.md) | 實施計劃 | 已完成（local verification；production/native gates deferred） | Revision 3 經兩個相同全範圍 reviewer PASS；48 個 normal forward migrations、guarded reset／reseed、roster/auth/invariant suites、fresh 4-test disposable admin workflow、PII／migration contract checks 已完成；production-only positive config、完整原生 screen-reader／device matrix仍 deferred |
| [teacher-workspace-roster-progress-redesign.md](./teacher-workspace-roster-progress-redesign.md) | 重設計計劃 | 已完成（local implementation／verification；external gates deferred） | Revision 3 經兩個獨立相同全範圍 review 及修訂；名冊／進度分家、學生詳情、搜尋／班級篩選／分頁、canonical班級洞察、default-false教師級 reset capability AND班級scope 已完成並由 fresh local browser／API suites驗證；不包括 production deploy、full-scale及native device matrix |
| [admin-user-directory-and-learning-analytics.md](./admin-user-directory-and-learning-analytics.md) | 實施計劃 | 已完成（local verification；production/native gates deferred） | admin directory／reset／analytics／繁體 demo fixture、EXPLAIN／scale budget及fresh 4-test admin roster workflow已驗證；production deploy、contract migration、VoiceOver／TalkBack及完整原生裝置QA deferred |
