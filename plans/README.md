...
| [word-catalog-governance-and-lifecycle.md](./word-catalog-governance-and-lifecycle.md) | 實施計劃 | 進行中（治理工作區第一階段 local implementation／verification） | 建立 sense-level 詞庫、39 欄 `word-catalog-v1`（兩個 prompt 欄保留但必須留空）、每方向 5–6 個人工干擾項及同詞跨 sense 正解排除、immutable approved revision、多人 conflict 檢查、老師 capability 四眼審核、一般停用／緊急撤回及可追溯 catalog revision；現已完成 local schema、逐條治理 API／UI 及初始 activation，CSV preview／commit、學習 cutover 及 production 仍待後續階段 |
| [csv-word-catalog-local-database-cutover.md](./csv-word-catalog-local-database-cutover.md) | 資料切換／實施計劃 | 進行中（Revision 2；雙 reviewer findings 已跟進） | 以 A1–B2 sense-level CSV 取代本地 Markdown canonical seed，加入逐表 word→sense transition、V1 read-only compatibility、test-only eligibility、安全 public question contract及真實解鎖 demo，再受保護地完整重建 local schema；destructive local reset 仍有獨立執行閘門 |
