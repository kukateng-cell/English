...
| [word-catalog-governance-and-lifecycle.md](./word-catalog-governance-and-lifecycle.md) | 實施計劃 | 進行中（完整詞庫 server pagination／search 已完成本地驗證） | 建立 sense-level 詞庫、39 欄 `word-catalog-v1`、分方向人工干擾池、immutable approved revision及老師 capability 單一獨立審核；一般申請由第一個終局決定生效，具權限者可留痕後立即軟停用。現已完成治理 API／UI、學生及統計 current reader、V1 projection、digest-bound 5,469 ACTIVE／107 DRAFT baseline、CSV preview／原子commit、audit history、catalog limiter及full-size本地效能加固；完整詞庫現由資料庫篩選、signed snapshot cursor及每頁最多100條提供；production rollout、managed環境效能／外部UAT及legacy cleanup仍待後續 |
| [word-catalog-bulk-submission-and-history.md](./word-catalog-bulk-submission-and-history.md) | 實施計劃 | 本地效能加固已完成 | response amplification已解決；200-row preview由逐組串行SQL改為transaction內批量快照及批量insert，三次本機p95為176.76–183.13 ms並通過所有local gates；staging／Vercel及production migration／deploy未執行 |
