...
- client 不再合併任何回執到 current item：核對 operation／action identity 後統一重新讀目前 scope 的 server state（含同題舊回執），bootstrap 用 generation／scope 防止過時查詢覆蓋。目前增加 reveal／answer 各一次權威 GET，換取不混用不同 session／item／revision；卡面、選項及 checkpoint 一起採 server 狀態。
...
- [ ] unit、lint、typecheck、build、V2 DB／browser、card-motion 回歸。
...
- 第一輪 card-motion 因本機 `.env.local` 強制 V2 all，V1 tests 等待 `/api/study` 而超時，已停止；明確 off 重跑中，未改動環境檔。
