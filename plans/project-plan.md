...
| **P7 內容完善及詞庫治理** | 🟡 第一階段 local implementation／verification | 已建立 39 欄 [`word-catalog-v1` 團隊編寫標準](./artifacts/word-catalog-authoring-standard-v1.md)及[詞庫生命週期實施計劃](./word-catalog-governance-and-lifecycle.md)；使用者已確認 Objective Probe 唔使用 prompt，題幹固定由 `term`／`definition_zh` 衍生，同詞其他 sense 正解不得成為干擾項；另納入 structured accepted answers、5–6 個人工干擾項、immutable approved revision、四眼審核、一般停用／緊急撤回及 catalog revision。現已完成 local catalog governance schema、初始 ACTIVE／DRAFT 整理、管理員／老師檢視與篩選、草稿提交、capability 審核／批准／拒絕及明確停用／重啟；現行學生學習／統計仍未完成 sense-level cutover | 後續完成 CSV preview／atomic commit、Markdown converter／零 silent-loss 報告、legacy mapping、學習／排行榜／分析 cutover及 production rollout；production 未開始 |
