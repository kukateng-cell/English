...
| API dispatch | `/api/study` remains the V1 route and retains its session/nonce validator | `/api/study/stream`, `/api/study/actions`, `/api/study/sessions/renew` are typed V2 boundaries | Route files, Phase 0 handoff addendum and same-learner dual-flow DB rehearsal |
