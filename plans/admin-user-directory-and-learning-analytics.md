...
- `npm run test:roster`、`npm run test:roster:invariants`、`npm run test:roster:lifecycle`、`npm run test:roster:auth`、`npm run test:roster:reset`、`npm run check:roster-pii`均通過；涵蓋班級權限隔離、raw DB invariants、hard-delete staging purge、session-bound recent auth、49-migration reset guard及PII／credential artifact scan；
